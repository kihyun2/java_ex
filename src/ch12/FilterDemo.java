...
public class FilterDemo {
	public static void main(String[] args) {
		...
		System.out.println();
		IntStream intOf = IntStream.of(1, 2, 2, 2, 3, 4, 4, 5, 6, 7);
		IntStream filterIntOf = intOf.filter(x -> x % 2 == 0);
		IntStream distinct = filterIntOf.distinct();
		distinct.forEach(Util::printWithParenthesis);
		System.out.println();

		System.out.println("인구가 1억 이상인 나라");
		Stream<Nation> nations = Nation.nations.stream();
//		List<Nation> nations = Nation.nations;
		Stream<Nation> nationFilter = nations.filter(n -> n.getPopulation() >= 100.00);
