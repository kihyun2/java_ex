...
public class Map2Demo {
	public static void main(String[] args) {
		...
		// 나라 이름만을 가지고 스트림으로 변환하고
		// 그중 4개의 나라이름만 ()로 감싸서 출력해보기
		...
		// 인구수를 ()로 감싸서 출력해보기
//		Nation.nations.stream().map(n -> n.getPopulation()).forEach(Util::printWithParenthesis);
		Nation.nations.stream().mapToDouble(n -> n.getPopulation()).forEach(Util::printWithParenthesis);
		System.out.println();

		// Nation.nations 리스트를 스트림으로 생성한 후
		// 각나라의 GDPRank 만을 가지고 스트림으로 변환하고
		// GdpRank를 ()로 감싸서 출력해보기
//		Nation.nations.stream().map(n -> n.getGdpRank()).forEach(Util::printWithParenthesis);
		Nation.nations.stream().map(n -> n.getGdpRank()).forEach(Util::printWithParenthesis);
